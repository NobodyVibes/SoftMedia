...
- [x] WS-6 T6.1/T6.2/T6.4/T6.7 ✅ — §7 Q6 DECIDED (maintainer 2026-07-18: "do it now"). Query-string auth on media routes accepts ONLY media/cast tokens (full access tokens are header-only); media tokens are GET/HEAD-only (hub negotiate exempt); SPA hard-depends on the media token (no fallback, gated cold load with retry, hub on media token); all mutating player calls → Authorization headers (sendBeacon → keepalive fetch); media-token minting requires read:library for API tokens (in-method — AuthController's class [AllowAnonymous] suppresses action [Authorize]) (+4 media-token tests; 2 file-serving tests migrated to media tokens). Adversarial review: 2 MED found + fixed (connecting-gate deadlock on non-401 server failure → log-out escape hatch after ~12s; BookReader's missed access-token-in-URL fallback → media-token-only). Live-verified end-to-end incl. playback lifecycle + beforeunload keepalive stop (network-log-proven)
- [x] §7 Q3 DECIDED (maintainer 2026-07-18): two-layer bitrate model confirmed ALREADY IMPLEMENTED — admins own server-wide/per-user caps; each device controls only its own bitrate (Client Settings → Playback, localStorage per user per device; server clamps device values to admin caps)


## Metadata & Cache Lifecycle (July 2026) — _✅ COMPLETE 2026-07-29; server 1990/0, client 577/0, build 0 warnings. Plans: `docs/plans/metadata-cache-followups-plan-2026-07-29.md`_
- [x] Library deletion cleans ALL derived artifacts — cast headshots keyed by `Person.ExternalId` (old code passed PKs = silent no-op) with shared-person guard, trickplay (had NO deletion path), thumbnails, cached subtitle VTTs (`LibraryCleanupService`); `LibraryRecentCaches` row purged
- [x] Image-proxy duplicates fixed at the root — `ProxyImageStore` deletes a URL's proxy copy (+ thumbs + `.404`) at download write-back and nightly adoption; 30-day TTL with touch-on-hit for strays; ComicSeries posters actually download now (queued-then-dropped before)
- [x] Daily sweep covers every artifact store (trickplay/thumbnails/cast/subtitles/proxy TTL) + reaps uncredited Person and unlinked Genre rows; row-existence contract kept (IsMissing artifacts heal on drive return)
- [x] MC-WI-001 — static `/cache/subtitles` + `/cache/trickplay` 404 unauthenticated (both consumed only via authorized endpoints; live-verified incl. existing-file 404); `/cache/images` stays public by design
- [x] MC-WI-002 — shared `ImageFetchPolicy`: ImageController's broad `.archive.org` suffix (admitted web.archive.org laundering) aligned to the audited L-26 anchored storage-node suffixes; SSRF tests both directions
- [x] MC-WI-003/004 — TrickplayWorker skips IsMissing; scan hard-deletes run the same per-item artifact cleanup as manual deletes (identity captured before `ExecuteDelete`)
- [x] MC-WI-007 — admin Cache Usage card (`GET /api/v1/admin/cache-stats`) with per-area files/bytes
- [x] MC-WI-008/009 — `/api/media/{id}/subtitles/{trackIndex}` reuses the persistent VTT cache; thumbnail touch-on-hit ends 7-day regen churn
- [x] MC-WI-010 + hygiene — BOTH dead test projects removed (`src/SoftMedia.Tests`, `tests/SoftMedia.Tests`) + `TempDebug`/`TempVerify`/`temp-net-test`; CONTRIBUTING corrected (single live suite, Tailwind semantic classes); last build warning fixed

## Artwork Authentication (July 2026) — _✅ COMPLETE 2026-07-29; server 2001/0, client 580/0, live-verified. Plan: `docs/plans/artwork-auth-plan-2026-07-29.md` (adversarially reviewed pre-implementation)_
- [x] AA-WI-011 (prerequisite) — `UserEligibilityCache`: the per-request media/cast-token DB recheck (audit L-3) now hits a 45s-TTL singleton cache, eagerly invalidated by ban/approve/deny/delete write paths (UsersController, AccountController) — removes a DB query per HLS segment and per poster
- [x] AA-WI-001/002 — `/cache/images/**` requires auth: explicit `AuthenticateAsync` in the static middleware (401, documented deviation from the 404 anti-probe rule), `/cache/images` added to `IsMediaRoute`; cast-token scope deliberately UNTOUCHED (review decision — cast poster carries the media token). Also closes the static bypass of `proxy/`+`thumbnails/`
- [x] AA-WI-003 — 10-case integration matrix (anon/media/cast/full-query/full-header/garbage/banned-eager-invalidation + subtitles/trickplay unchanged); the 2 pre-existing L-3 ban tests updated to invalidate the cache like the real ban endpoint
- [x] AA-WI-004/005 — `attachAuthToApiUrl` widened to `/cache/images/`; ALL client `/cache` bypasses deleted (CastStripItem, TVDetailView ×4, Book/ComicSeries/MovieDetailView) and the five duplicated `getImageUrl` helpers collapsed into shared `resolveArtworkUrl`; grep gate: only the helper mentions `/cache`
- [x] AA-WI-006/007 — cast poster + mediaSession artwork tokenized via the shared helper; detail views subscribe to `useMediaTokenRefresh`; client tests updated + new helper coverage
- [x] AA-WI-008/009 — live probes green (anon poster 401, proxy subdir 401, subtitles/trickplay 404); CHANGELOG + native-client onboarding doc updated (artwork needs the media token). Interactive Chromecast/lock-screen checks deferred to next real viewing session (cosmetic-only risk); AA-WI-010 post-week sweep open
