...
SoftMedia automatically manages temporary transcoding files to prevent disk space issues while supporting resume playback.

## Automatic Cleanup Triggers

### 1. Video Completion
When a video finishes playing naturally (reaches the end), the temporary files for that video are deleted immediately.
