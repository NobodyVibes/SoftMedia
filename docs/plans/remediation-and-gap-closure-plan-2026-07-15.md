...
3. **R-WI-009 bitrate cap** (roadmap Q3): ✅ **DECIDED (maintainer, 2026-07-18): two layers — admins own the server-wide/per-user caps; users control their own bitrate PER DEVICE** (a device changes only itself, never the account's other devices). Verified ALREADY IMPLEMENTED end-to-end: Settings → Client Settings → Playback has per-device "Maximum bitrate" (+ data-saver, default quality; audio has its own) stored in this device's localStorage per user; the player folds it into the stream-plan request; `StreamPlanService` honors it only DOWNWARD (a device value above the admin cap is clamped — `StreamPlanService.cs:166-169`). No code change required.
...
6. **R-WI-002/005 × WS-6 sequencing**: ✅ **DECIDED (maintainer, 2026-07-18): "do it now" — landed after the plan-store work as its own item.** T6.1 (query-string tokens must be media/cast — full access tokens are header-only), T6.2 (SPA hard-depends on the media token: `getUrlToken()` has no access-token fallback, the authed UI gates on media-token resolution with retry, the SignalR hub rides the media token), and T6.4 (media tokens are GET/HEAD-only; every mutating transcode call + the debug POST moved to Authorization headers; the beforeunload stop beacon became a keepalive fetch since sendBeacon can't set headers). Plus a WS-6×B-18 interaction fix: media-token minting now requires `read:library` for API tokens (in-method gate — AuthController's class-level `[AllowAnonymous]` suppresses action `[Authorize]` attributes). See `security-hardening-wave-2-implementation-plan.md` WS-6.
...
| 2026-07-18 | **Post-Phase-C bug-fix wave COMPLETE (baseline commit `b0d63a2`).** All 21 backlog engineering items fixed except parked B-18 (§7 Q1): B-01 (cap gates direct-play/remux at plan time + serve-time 403), B-02 (fabricated-sid resolution/codec clamps), B-03…B-12, B-13/14 (compliant `subtitles.m3u8` rendition, DEFAULT=NO), B-15/16/17, B-19 (hero rating ceiling), B-20/21, T-01 (test factory → per-scope shared-cache SQLite; two consecutive clean full parallel runs). Adversarial review of the whole diff found 1 HIGH (B-15 guard skipped old-`<track>` cleanup when switching to Off → hoisted out) — fixed + re-verified. Live-verified: B-03/05/07/12/13/14 + far-seek VTT offset. Server 1070/1/0; client 237; tsc clean; production CSS build verified. Details in `post-phase-c-bug-backlog.md` §Fix wave summary. | Engineering execution |
...
**Checkpoint 11 — 2026-07-18.** **Post-Phase-C bug-fix wave COMPLETE.** Baseline: commit `b0d63a2` (the full R-WI-001..020 branch state). Executed as four waves — server hardening (B-01/02/19/20/21), subtitles/HLS (B-13/14/15/16/17), client polish (B-03/04/07/08/09/10/11), search/theme/test-infra (B-05/06/12, T-01) — each with tests, then one adversarial review across the whole diff and live verification against the running app. Full close-out with per-fix detail lives in `docs/plans/post-phase-c-bug-backlog.md` (§ Fix wave summary); highlights:

- **B-01/B-02 (playback hardening):** the per-user bitrate cap now gates direct play AND remux at plan time (`SourceFitsBitrateCeiling`, reason code `bitrate.cap-forces-transcode`) with a serve-time 403 backstop on `/stream/{id}` for over-cap video; fabricated-sid `master.m3u8` clamps resolution (`ResolutionRank`) and codec to server settings.
- **B-13/B-14:** the HLS master's subtitle rendition now references a compliant single-segment WebVTT VOD playlist (new `GET /api/transcode/{id}/subtitles.m3u8`, `no-store`) with `DEFAULT=NO,AUTOSELECT=NO` — hls.js stops mis-parsing the raw .vtt and native/iOS players gain manual subtitles without double-rendering on the web client.
- **T-01:** root-caused the CPU-contention flakes — the test factory handed ONE open SQLite connection to every EF scope and every hosted service (same-connection transaction collisions `busy_timeout` can't fix). Rewritten to a uniquely-named `Mode=Memory;Cache=Shared` DB with per-scope connections + a keep-alive pin. Two consecutive clean full parallel runs.
- **Review finding (HIGH, fixed):** the B-15 guard skipped the old-`<track>` cleanup when the user switched subtitles to Off, leaving the previous track's cues rendering — cleanup hoisted out of the guard; suites re-run green.
- **Live-verified:** episode search in the TV library grid (B-05), album tracks carrying artist/album name context (B-03), the no-results dropdown + valid row markup (B-07), `@theme` palette resolving in the running app + production CSS emitting the previously dead utilities (B-12), the full master→subtitles.m3u8→.vtt chain (B-13/14), and the far-seek VTT offset (post-seek cues shifted stream-relative, pre-seek cues dropped). All verification state reset (transcode sessions deleted, temp dirs removed, no DB residue).
- **Parked:** B-18 (`read:library` scope enforcement) awaits the §7 Q1 decision — enforcing now would break existing tokens.

Suites at close: **server 1070 pass / 1 skip / 0 fail; client 237 pass; `tsc --noEmit` clean; production client build verified.** New tests this wave: `StreamPlanServiceBitrateTests` (+2), `StreamBitrateCapIntegrationTests` (2), `TranscodeResolutionClampTests` (13), `HeroRatingCeilingIntegrationTests` (1), `SubtitleRenditionTests` (5), `AlbumTracksNameContextTests` (1), `GlobalSearchIntegrationTests` (+2).
