...
| WS‑10 | ◑ partial | **M-3** (atomic 2FA lockout — closes the brute-force race), **I-5** (atomic refresh-token rotation — no chain fork), L-7 (password policy on CreateUser), L-8 (disable-path lockout), I-4 (enroll rate limit) done. **L-9/L-10/I-2 deferred** — L-10 recovery-code KDF invalidates existing codes (migration); L-9 escalating backoff is low-value; I-2 enumeration is rate-limited with a UX tradeoff. |
...
> - **Needs concurrency load-testing**: M-5 atomic transcode cap (L-14's hard ceiling already bounds the race to a small finite overage).
