...
> - **Needs the operator's live environment** (can't be verified headless / no Cast device here, and the enforce decision is the operator's): flipping `Security:EnforceCsp` to enforcing (report-only is shipped + verified against the build) + token-in-memory (T13.2); WS-6 T6.1/T6.2/T6.4 query-token rejection + SPA media-token migration (high blast radius — media playback); L-18 scan-progress scoping (the app-wide toast depends on the `Clients.All` broadcast).
...
> - **Low value / tradeoff**: L-9 escalating 2FA backoff (codes rotate every 30s, so the per-window cap already protects); I-2 signup-enumeration uniform response (rate-limited; UX tradeoff); I-9 full ArgumentList migration (MediaPathSafety already closes the H-2 vector); I-3 DB-backed must_change; L-24 hub group re-check on revocation.
>
> **Test-infra note (not a security finding):** the full parallel suite occasionally shows 1–2 intermittent failures (a SQLite "active statements" / FK-on-teardown race in the in-memory/file-SQLite integration tests under xunit parallelism). They pass on isolation/retry and are unrelated to these changes; a follow-up could put the SQLite-backed integration tests in a single non-parallel xunit `[Collection]` or ensure `SqliteConnection.ClearAllPools()` on teardown.
