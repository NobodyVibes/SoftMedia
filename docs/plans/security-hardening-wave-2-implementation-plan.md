...
| WS‑6 | ✅ done 2026-07-21 (T6.5/T6.6 closed in Session 1 closeout) | T6.1 (query tokens must be media/cast), T6.2 (SPA hard-depends on the media token incl. the hub), T6.3 (live user-state recheck), T6.4 (media token GET/HEAD-only + client writes moved to headers), T6.7 (tests) all landed — maintainer green-lit via remediation-plan §7 Q6. T6.5 (proxy scheme check) and T6.6 (log scrubbing) remain open as ordinary backlog. |
...
| WS‑10 | ◑ partial | **M-3** (atomic 2FA lockout), **I-5** (atomic refresh-token rotation), **L-10** (recovery-code entropy 40→80 bits), L-7 (password policy on CreateUser), L-8 (disable-path lockout), I-4 (enroll rate limit) done. **L-9/I-2 deferred** — L-9 escalating backoff is low-value (TOTP codes rotate every 30s, so the per-window cap already protects); I-2 signup enumeration is rate-limited (15/min/IP) with a real UX tradeoff (silent no-op on a typo'd existing username). |
| WS‑11 | ✅ done | L-11/L-12 — webhook SSRF classifier blocks unspecified address (0.0.0.0/::) + CGNAT 100.64/10. |
| WS‑12 | ✅ done 2026-07-21 | L-19 (rate limiter after auth) + **L-23** (SignalR Join throttle) done. **L-18 closed in Session 1 closeout**: the `981e387` refactor removed the app-wide toast (the original deferral reason); the only ScanProgress consumers are admin-gated, so the broadcast now targets a `scan-admins` group populated at hub connect via DB role lookup (media tokens carry no role claim by design). L-24 (hub ACL re-check on revocation) remains deferred — low value now that scan-progress is scoped and Join* re-checks ACL per call. |
| WS‑13 | ◑ partial | **CSP ENFORCING as of 2026-08-02** (`Security:EnforceCsp` default true in appsettings). Prerequisite landed the same day: the server now hosts the built SPA (wwwroot fallback + `npm run deploy:server`), so the header finally reaches the document. Audited under enforcement against the server-hosted build in a live browser: login, home (SignalR ws + media-token images + service worker), HLS transcode playback (blob media, hls.js), HDR pre-play prompt, PDF reader (pdf.js blob worker), EPUB reader (srcdoc iframe), settings, account, Quick Connect pairing — zero violations after ONE real fix: the gstatic source is now SCHEME-LESS (`www.gstatic.com`) because the Cast SDK chain-loads its framework over `http://` on http-served pages (https-anchored source blocked it; on https deployments mixed-content blocking already forbids http scripts, so nothing is loosened). Two caveats recorded: PWA clients pick up header changes only when the service-worker precache updates (cached responses carry cached headers), and a REAL Cast-device pass stays on the parked hardware list. **T13.2 (access token → in-memory) remains open** — deliberately NOT bolted onto this pass: it is an auth-store refactor (reload = silent cookie refresh) that deserves its own focused session with regression tests; CSP enforcement has meanwhile shrunk the XSS window it defends against. |
...
> **Remaining work, by why it's deferred (updated 2026-08-02 — CSP flip DONE, see WS-13):**
> - **Own focused session**: T13.2 token-in-memory (auth-store refactor + silent-refresh-on-reload; regression-sensitive, so not bolted onto the CSP pass — see WS-13 row).
> - **Needs concurrency load-testing**: M-5 atomic transcode cap (L-14's hard ceiling already bounds the race to a small finite overage; a botched atomic fix could deadlock streaming).
> - **Low value / tradeoff (L-24 DECIDED 2026-08-02: accepted residual risk)** — an admin demoted mid-connection keeps the scan-admins hub group only until reconnect; L-18's scoping means the group carries admin-only scan telemetry, not media, and Join* re-checks ACL per call. Revisit only if hub groups ever carry sensitive payloads. Others unchanged: L-9 escalating 2FA backoff (codes rotate every 30s, so the per-window cap already protects); I-2 signup-enumeration uniform response (rate-limited; UX tradeoff); I-9 full ArgumentList migration (MediaPathSafety already closes the H-2 vector); I-3 DB-backed must_change.
>
> **Test-infra note — RESOLVED 2026-07-21:** the intermittent SQLite "active statements" / FK-on-teardown failures were caused by `DlnaIntegrationTests` and `ForwardedHeadersIntegrationTests` hosting full apps OUTSIDE the serialized "Integration" collection (they subclass the factory directly instead of deriving from `IntegrationTestBase`, so they ran in parallel with the other integration hosts). Both now carry `[Collection("Integration")]` explicitly.
...
- [x] **T6.1** ✅ 2026-07-18 — **M-2:** `OnMessageReceived` marks query-lifted tokens (`HttpContext.Items[QueryTokenSourceKey]`); `OnTokenValidated` fails any query-sourced token whose `token_use ∉ {media, cast}`. Full access tokens are header-or-nothing. Control test proves the same token still authenticates via the Authorization header.
- [x] **T6.2** ✅ 2026-07-18 — **M-2 (SPA):** `getUrlToken()` returns the media token ONLY (no access-token fallback); App.tsx gates the authed UI on media-token resolution ("Connecting to your library…", 4s retry loop); the SignalR hub connects with the media token via a call-time `accessTokenFactory` (gated on a boolean so the ~15-min token rotation doesn't tear down the connection); `getAccessToken()` added for header-only use.
- [x] **T6.3** — **L-3:** re-validate the media token against live user state (`IsBanned/IsDeleted/IsApproved`) in `OnTokenValidated`, mirroring the cast-token recheck (coordinate with T3.4).
- [x] **T6.4** ✅ 2026-07-18 — **L-4/L-5:** media tokens now require `GET`/`HEAD` (the `/hubs/media` prefix is exempt — the SignalR negotiate handshake is a POST; hub methods are guarded by the hub itself). Client counterpart: ALL mutating transcode calls (session DELETE ×8, pause/resume, plan POSTs, debug POST) send the ACCESS token in an Authorization header via `transcodeAuthHeaders()`; the beforeunload `sendBeacon` (can't set headers) became a `keepalive` fetch. **WS-6×B-18 interaction closed:** media-token minting requires `read:library` for API tokens — enforced IN-METHOD because AuthController's class-level `[AllowAnonymous]` suppresses action `[Authorize]` attributes (the attribute is kept as defense-in-depth).
- [x] **T6.5** ✅ 2026-07-21 — **I-8:** the INITIAL url now passes the same http(s) scheme guard as redirect hops, at BOTH entry points: `ImageCacheService.CacheImageAsync` and `ImageController.ProxyImage`. Tests: 5 new theory cases (ftp/file/gopher on allowlisted hosts) assert rejection with zero network activity.
- [x] **T6.6** ✅ 2026-07-21 — **T4.1 from wave 1 deferral:** audit found the app code never logs query strings (the AuthController cookie diagnostic is names-only by design); the one leak path was `Microsoft.AspNetCore.Hosting.Diagnostics` logging full request URLs (`?token=<JWT>` on media routes) if an operator raises log levels for debugging. That category is now explicitly pinned to Warning in BOTH appsettings files — the more-specific pin survives a raised `Default`/`Microsoft.AspNetCore` (important once NR-WI-011 adds an in-app LogLevel control). Guarded by `LoggingTokenScrubConfigTests`.
- [x] **T6.7** ✅ 2026-07-18 — Tests: role-bearing JWT in `?token=`/`?access_token=` rejected on media routes (header control passes); media token in query still authenticates; media token refused on transcode DELETE + book-write POST (GET control passes); unscoped API token refused a media-token mint. `FileServingControllerIntegrationTests`' query-token tests migrated to media tokens (the token the SPA actually uses now). +4 in `MediaTokenIntegrationTests`.
