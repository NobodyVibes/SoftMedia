...
| WS‑13 | ◑ partial | **CSP shipped report-only** (tuned policy + `Security:EnforceCsp` flag, default off). Verified against the Vite production build: no inline scripts, and the policy now allows `https://www.gstatic.com` for the Google Cast SDK (would otherwise break Cast when enforced). **Flipping to enforcing + token-in-memory (T13.2) deferred** — needs a live SPA run (and a real Cast device) to confirm no residual runtime violations before enabling the flag; report-only surfaces any remainder in the operator's environment. |
...
> **Remaining work, by why it's deferred (updated 2026-07-21 after the Session 1 closeout):**
> - **Needs the operator's live environment**: flipping `Security:EnforceCsp` to enforcing (report-only is shipped + verified against the build) + token-in-memory (T13.2). *(WS-6 T6.1/T6.2/T6.4 landed 2026-07-18; T6.5/T6.6 and L-18 closed 2026-07-21 — see the workstream table.)*
...
> - **Low value / tradeoff**: L-9 escalating 2FA backoff (codes rotate every 30s, so the per-window cap already protects); I-2 signup-enumeration uniform response (rate-limited; UX tradeoff); I-9 full ArgumentList migration (MediaPathSafety already closes the H-2 vector); I-3 DB-backed must_change; L-24 hub group re-check on revocation (reduced further by L-18's scoping).
>
