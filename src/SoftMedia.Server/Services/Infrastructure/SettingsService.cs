...
public class SettingsService : ISettingsService
{
    ...
    public async Task InitializeDefaultsAsync()
    {
        var defaults = new List<AppSetting>
        {
            ...
            // Scanning
            new() { Key = "EnableFileWatcher", Value = "true", Group = "Scanning", Description = "Automatically detect new files and update library. Disable for manual scanning only. Requires a server restart to take effect." },
            new() { Key = "LibraryScanIntervalHours", Value = "12", Group = "Scanning", Description = "Hours between automatic scans of ALL libraries. 0 = disabled. A backstop for changes the realtime file watcher can miss (network shares, removable drives). The first scan runs shortly after enabling; changes take effect within a few minutes without a restart. Libraries already being scanned are skipped, not double-scanned." },
            new() { Key = "MaxScanPurgePercent", Value = "25", Group = "Scanning", Description = "Safety brake: if a scan finds that more than this percentage of a library's files have vanished at once (and at least 20 items), nothing is marked missing and the scan raises a warning instead — the usual cause is an unmounted drive or dropped network share, not real deletions. Set to 100 to disable the brake for one scan after intentionally mass-deleting files." },
            new() { Key = "MissingItemRetentionDays", Value = "30", Group = "Scanning", Description = "Days a missing item (file vanished from disk) is kept — hidden but with watch history, ratings and playlist membership intact — before it is permanently deleted. If the file reappears within the window, everything is restored. 0 = delete immediately (old behavior, not recommended)." },
