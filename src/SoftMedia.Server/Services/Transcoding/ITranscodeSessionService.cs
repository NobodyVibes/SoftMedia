...
public interface ITranscodeSessionService
{
    ...
    /// </summary>
    void UpdateClientPosition(Guid mediaId, Guid userId, int? sub, string segment);

    /// <summary>
    /// Pauses the transcode session.
    /// </summary>
    /// <returns>True if successful, false if session not found or unauthorized.</returns>
    TranscodeSessionResult PauseSession(Guid mediaId, Guid userId, int? sub);

    /// <summary>
    /// Resumes the transcode session.
    /// </summary>
    /// <returns>True if successful, false if session not found or unauthorized.</returns>
    TranscodeSessionResult ResumeSession(Guid mediaId, Guid userId, int? sub);

    /// <summary>
    /// Stops a specific transcode session.
    /// </summary>
    void StopSession(Guid mediaId, Guid userId, int? sub);
