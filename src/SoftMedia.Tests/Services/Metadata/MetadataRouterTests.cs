...
public class MetadataRouterTests
{
    ...
    public async Task FetchMetadataAsync_SelectsCorrectProvider()
    {
        ...
        movieProvider.Setup(p => p.FetchMetadataAsync(It.IsAny<string>())).ReturnsAsync("MovieData");

        var tvProvider = new Mock<IMetadataProvider>();
        tvProvider.Setup(p => p.SupportedType).Returns(LibraryType.TV);
        tvProvider.Setup(p => p.FetchMetadataAsync(It.IsAny<string>())).ReturnsAsync("TVData");

        var router = new MetadataRouter(new[] { movieProvider.Object, tvProvider.Object });

        // Act
        var movieResult = await router.FetchMetadataAsync("Matrix", LibraryType.Movie);
        var tvResult = await router.FetchMetadataAsync("Friends", LibraryType.TV);
    ...
    public async Task FetchMetadataAsync_ReturnsNull_WhenNoProviderFound()
    {
        ...
        // Act
        var result = await router.FetchMetadataAsync("Unknown", LibraryType.Book);
